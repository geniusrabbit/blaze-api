...
func (mng *Manager) roleByModel(role *model.Role, roles map[uint64]rbac.Role, perms map[uint64]rbac.Permission, links []*model.M2MRole) (rbac.Role, error) {
	...
	return rbac.NewRole(role.Name, rbac.WithChildRoles(roleList...),
		rbac.WithSubPermissins(permList...),
		rbac.WithExtData(&ExtData{
			ID:     role.ID,
...
func (mng *Manager) NewResourcePermission(name string, resType any, options ...rbac.Option) (rbac.Permission, error) {
...
func (mng *Manager) MustNewResourcePermission(name string, resType any, options ...rbac.Option) rbac.Permission {
	perm, err := mng.NewResourcePermission(name, resType, options...)
	if err != nil {
		panic(err)
	}
	return perm
