...
func (s *testSuite) SetupSuite() {
	...
	s.authclientRepo = NewAuthclientRepository()
}
...
func (s *testSuite) TestFetchList() {
	...
	clients, err := s.authclientRepo.FetchList(s.Ctx, &authclient.Filter{ID: []string{"1", "2"}})
	s.NoError(err)
...
func (s *testSuite) TestCount() {
	...
	count, err := s.authclientRepo.Count(s.Ctx, &authclient.Filter{ID: []string{"1", "2"}})
	s.NoError(err)
